...
{
	int i;
	for (i = 0; i < matrix->rowCount; i++)//assuming the matrix is squar
	{
		matrix->data[i][i] = v;
	}
...
ILDA_bool CONCAT2(ILDA_MATRIX(ILDA_matrix), init_default)(ILDA_MATRIX(ILDA_matrix)* matrix)
{
...
//operators
ILDA_bool CONCAT2(ILDA_MATRIX(ILDA_matrix), add)(ILDA_MATRIX(ILDA_matrix)* augend, const ILDA_MATRIX(ILDA_matrix)* addend)
{
	size_t i, j;
...
ILDA_bool CONCAT2(ILDA_MATRIX(ILDA_matrix), sub)(ILDA_MATRIX(ILDA_matrix)* minuend, const ILDA_MATRIX(ILDA_matrix)* subtrahend)
{
	size_t i, j;
...
ILDA_bool CONCAT2(ILDA_MATRIX(ILDA_matrix), mul_same)(ILDA_MATRIX(ILDA_matrix)* multiplicand, const ILDA_MATRIX(ILDA_matrix)* multiplier)
{
	size_t i, j, k;
